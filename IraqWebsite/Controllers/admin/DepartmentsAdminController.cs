...
namespace IraqWebsite.Controllers
{
	...
	public class DepartmentsAdminController : Controller
	{
		...
		public DepartmentsAdminController(ApplicationDbContext context, IMapper mapper, IWebHostEnvironment webHostEnvironment)
		{
			_context = context;
			_mapper = mapper;
		...
		{
			var section = await _context.Departments.ToListAsync();
			return View(section);
		}
		...
		{
			var aboutUs = await _context.Departments.FindAsync(id);
			if (aboutUs == null)
			{
				return RedirectToAction(nameof(Index));
			}
			var aboutUsVm = _mapper.Map<EditDepartmentDto>(aboutUs);
		...
		public async Task<IActionResult> Create(CreateDepartmentDto model)
		{
			...
			{
				var service = _mapper.Map<Department>(model);
		...
		[HttpPost]
		public async Task<IActionResult> Edit(EditDepartmentDto model)
		{
			if (ModelState.IsValid)
			{
				var service = _mapper.Map<Department>(model);
				var StatiscCurrent = await _context.Departments.AsNoTracking().FirstOrDefaultAsync(x => x.Id == model.Id);
		...
		public async Task<IActionResult> DeleteConfirmed(int id)
		{
			...
			if (aboutUs != null)
			{
				_context.Departments.Remove(aboutUs);
			}
